...
public class TflitePlugin implements FlutterPlugin, MethodCallHandler {
  ...
  private void setupChannel(BinaryMessenger messenger) {
    channel = new MethodChannel(messenger, "tflite");
  ...
  private AssetManager resolveAssetManager() {
    if (pluginBinding != null) {
      return pluginBinding.getApplicationContext().getAssets();
    }
  ...
  private String resolveAssetKey(String asset) {
    if (pluginBinding != null) {
      return pluginBinding.getFlutterAssets().getAssetFilePathByName(asset);
    }
    return asset;
  }
  ...
  private String loadModel(HashMap args) throws IOException {
    ...
    if (isAsset) {
      assetManager = resolveAssetManager();
      key = resolveAssetKey(model);
